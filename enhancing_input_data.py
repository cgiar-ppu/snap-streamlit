...
def get_action_area(submitter, initiative_to_aa):
    """
    Get Action Area from a submitter code
    """
    ...
    # Return Action Area if submitter exists in mapping
    return initiative_to_aa.get(submitter)

def main():
    # Define direct mapping from initiatives to Action Areas
    initiative_to_aa = {
        ...
        'INIT-10': 'RAFS',  # Fragility to Resilience
        'INIT-11': 'RAFS',  # Excellence in Agronomy
        'INIT-12': 'RAFS',  # Nature-Positive Solutions
        'INIT-13': 'RAFS',  # Plant Health
        'INIT-14': 'RAFS',  # AgriLAC Resiliente
        'INIT-15': 'RAFS',  # Aquatic Foods
        'INIT-16': 'RAFS',  # Resilient Cities
        'INIT-17': 'RAFS',  # Sustainable Animal Productivity
        'INIT-18': 'RAFS',  # Asian Mega-Deltas
        'INIT-19': 'RAFS',  # Mixed Farming Systems
        'INIT-20': 'RAFS',  # Transforming Agrifood Systems in South Asia
        'INIT-21': 'RAFS',  # Diversification in E & S Africa
        'INIT-22': 'RAFS',  # W & C African Food Systems Transformation
    ...
    data_df['Action Area'] = data_df['Submitter'].apply(
        lambda x: get_action_area(x, initiative_to_aa)
    )
